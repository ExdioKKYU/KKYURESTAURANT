...
public class Gimbob_attack_enemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Ability[] Abilityes = GameObject.FindObjectsOfType<Ability>();

        // 모든 적에 대해 반복
        foreach (Ability unit in Abilityes)
        {
            // 추가: 같은 열에 위치한지 확인
            if (Mathf.Approximately(transform.position.y, unit.transform.position.y))
            {
                // 해당 스크립트를 가지고 있는 오브젝트와의 거리를 계산
                float closeDistance = Vector3.Distance(transform.position, unit.transform.position);
